...
#' v <- runif(100)
#' R0 <- matrix(1/9, 3, 3)
#' result <- fit.copula.point(u, v, R0)
#' result$R
fit.copula.point <- function(u, v, R_init, prior = NULL, options = list()) {
  ...
  for (iter in seq_len(opts$maxiter)) {
    ...
    llf <- bernstein_estep(u, v, R, prior, tau_bar)
...
fit.copula.point.joint <- function(x, y, R_init, prior = NULL,
  Fx, Gy, options = list()) {
  stopifnot(is.matrix(R_init))
  m <- nrow(R_init)
  n <- ncol(R_init)
  ...
  for (iter in seq_len(opts$maxiter)) {
    ...
    llf <- bernstein_estep_with_weight(u, v, du, dv, R, prior, tau_bar, exw1, exw2, eyw1, eyw2)
